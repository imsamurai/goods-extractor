...
function TemplateExtractor(fieldSet, xpathExtractor) {
    ...
    function getFields() {
        ...
        for (var n in fs.data) {
            var untypedFields = fs.data[n].filter(function(field) {
                return field !== null
            });
            var typedFields = untypedFields.filter(function(field) {
                return field.type != 'text'
            });
            if (typedFields.length>0) {
                fields[typedFields[0].type] = typedFields[0];
            } else if (untypedFields.length > 0) {
                fields[untypedFields[0].name] = untypedFields[0];
            }
        }
        return fields;
    }
